window.addEventListener("load", () => {
    ...
    type categoryCollection = {
        ...
        questions: {
            [questionIndex:number]: {
                prompt: string;
                promptMathJax?:[mathJax:string,prompt:string];
    ...
    const categories: categoryCollection = [
        ...
        },
        {
            ...
            questions: [
                ...
                { prompt:"tan^-1", answer:["cos/sin","1/tan"]},
                { prompt:"cot", answer:["cos/sin","1/tan"]},
        ...
        category:"Limiti notevoli",
            questions:[
                { prompt:"per x->0, Lim sin(x)/x",  answer:"1" },
                { prompt:"per x->0, Lim ln(1+x)/x", answer:"1" },
                { prompt:"per x->0, Lim tan(x)/x", answer:"1" },
                { prompt:"per x->0, Lim (e^x-1)/x", answer:"1" },
                { prompt:"per x->0, Lim arctan(x)/x", answer:"1" },
                { prompt:"per x->0, Lim arcsin(x)/x", answer:"1" },

                { prompt:"per x->0, Lim sin(x)", answer:"x" },
                { prompt:"per x->0, Lim ln(1+x)", answer:"x" },
                { prompt:"per x->0, Lim tan(x)", answer:"x" },
                { prompt:"per x->0, Lim (e^x-1)", answer:"x" },
                { prompt:"per x->0, Lim arctan(x)", answer:"x" },
                { prompt:"per x->0, Lim arcsin(x)", answer:"x" },

                { prompt:"per x->0+, Lim xln(x)", answer:"0"},
                { prompt:"per x->0, Lim (a^x-1)/x", answer:["ln(a)","log(a)"]},
                { prompt:"per x->0, Lim (1-cos(x))/x^2", answer:"1/2"},

                { prompt:"per x->infinito, Lim (1+1/x)^x", answer:"e" },
                { prompt:"per x->infinito, Lim (1+x)^-x", answer:"e" },
        ...
        category:"Serie note",
            questions:[
                { prompt:"Σq^n, diverge per q", promptMathJax:["\\sum_{n=1}^{\\infty} q^n","diverge per q"], answer:"q≥1" },
                { prompt:"Σq^n, converge per q", promptMathJax:["\\sum_{n=1}^{\\infty} q^n","converge per q"],answer:"-1<q<1" },
                { prompt:"Σq^n, a cosa converge se -1<q<1", promptMathJax:["\\sum_{n=1}^{\\infty} q^n","se -1<q<1, a cosa converge"], answer:"1/(1-q)" },
                { prompt:"Σq^n, è indeterminata per q", promptMathJax:["\\sum_{n=1}^{\\infty} q^n","è indeterminata per q"], answer:"q≤-1" },
                { prompt:"Σ1/n^a diverge per a", answer:"a≤1" },
                { prompt:"Σ1/n(ln(n))^a diverge per a", answer:"a≤1" },
                { prompt:"Σ1/n^a converge per a", answer:"a>1" },
                { prompt:"Σ1/n(ln(n))^a converge per a", answer:"a>1" },
            ]
        },
        {
    ...
    function updateQuestionUI() {
        ...
        _inputTitle.textContent = "Risposta";
        if (_promptMathJax&&Array.isArray(_promptMathJax)) {
            try {
                /** @ts-ignore */
                _prompt.innerHTML = MathJax.tex2mml(_promptMathJax[0], {}) + "<p>" + _promptMathJax[1] + (typeof answer === "string" ? " = ?" : " =* ?") + "</p>";
            }
            catch(e) {
                console.log(e);
